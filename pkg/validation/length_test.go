...
func Test_LengthRule_Error(t *testing.T) {
	ctx := context.Background()
	r := Length(10, 20)
	assert.Equal(t, "the length must be between 10 and 20", r.Validate(ctx, "abc").Error())

	r = Length(0, 20)
	assert.Equal(t, "the length must be no more than 20", r.Validate(ctx, make([]string, 21)).Error())

	r = Length(10, 0)
	assert.Equal(t, "the length must be no less than 10", r.Validate(ctx, [9]string{}).Error())
