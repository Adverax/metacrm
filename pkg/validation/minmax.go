...
type thresholdRuleOptions struct {
...
func Min(min interface{}) ThresholdRule {
	return ThresholdRule{
		thresholdRuleOptions: thresholdRuleOptions{
			Threshold: min,
...
func (r ThresholdRule) RuleType() RuleType {
	return ThresholdRuleType
}

func (r ThresholdRule) MarshalJSON() ([]byte, error) {
	return json.Marshal(r.thresholdRuleOptions)
}

func (r *ThresholdRule) UnmarshalJSON(data []byte) error {
	if err := json.Unmarshal(data, &r.thresholdRuleOptions); err != nil {
		return err
	}
	...
	case greaterEqualThan:
		r.err = ErrMinGreaterEqualThanRequired
	case lessThan:
		r.err = ErrMaxLessThanRequired
	case lessEqualThan:
		r.err = ErrMaxLessEqualThanRequired
	default:
...
func (r ThresholdRule) Validate(_ context.Context, value interface{}) error {
	...
	if isNil || IsEmpty(value) && IsEmpty(r.Threshold) {
		return nil
	}
...
func init() {
	RegisterUnmarshaller(ThresholdRuleType, func(data []byte) (RuleEx, error) {
		rule := ThresholdRule{}
