...
func Length(min, max int) LengthRule {
	return LengthRule{
		lengthRuleOptions: lengthRuleOptions{
			Min: min,
			Max: max,
		},
		condition: true,
...
type lengthRuleOptions struct {
	Min  int  `json:"min,omitempty"`
	Max  int  `json:"max,omitempty"`
	Rune bool `json:"rune,omitempty"`
}
...
func (r LengthRule) RuleType() RuleType {
	return RuleTypeLength
}

func (r *LengthRule) MarshalJSON() ([]byte, error) {
	return json.Marshal(r.lengthRuleOptions)
}

func (r *LengthRule) UnmarshalJSON(data []byte) error {
	if err := json.Unmarshal(data, &r.lengthRuleOptions); err != nil {
		return err
	}

	r.condition = true
	r.err = buildLengthRuleError(r.Min, r.Max)
...
func (r LengthRule) Validate(_ context.Context, value interface{}) error {
	...
	if isNil || IsEmpty(value) && r.Min == 0 {
		return nil
	}
...
func init() {
	RegisterUnmarshaller(RuleTypeLength, func(data []byte) (RuleEx, error) {
		rule := Length(0, 0)
