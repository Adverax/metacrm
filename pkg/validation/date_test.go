...
func TestDateRule_Error(t *testing.T) {
	r := Date(time.RFC3339)
	ctx := context.Background()
	assert.Equal(t, "must be a valid date", r.Validate(ctx, "0001-01-02T15:04:05Z07:00").Error())
	r2 := r.Min(time.Date(2000, 1, 1, 1, 1, 1, 0, time.UTC))
	assert.Equal(t, "the date is out of range", r2.Validate(ctx, "1999-01-02T15:04:05Z").Error())
	r = r.Error("123")
	r = r.RangeError("456")
...
func TestDateRule_ErrorObject(t *testing.T) {
	...
	assert.Equal(t, "abc", r.Validate(ctx, "0001-01-02T15:04:05Z07:00").Error())
...
func TestDateRule_MinMax(t *testing.T) {
	r := Date(time.ANSIC)
	assert.True(t, r.dateRuleOptions.Min.IsZero())
	assert.True(t, r.dateRuleOptions.Max.IsZero())
	r = r.Min(time.Now())
	assert.False(t, r.dateRuleOptions.Min.IsZero())
	assert.True(t, r.dateRuleOptions.Max.IsZero())
	r = r.Max(time.Now())
	assert.False(t, r.dateRuleOptions.Max.IsZero())

	ctx := context.Background()
	r2 := Date("2006-01-02").Min(time.Date(2000, 12, 1, 0, 0, 0, 0, time.UTC)).Max(time.Date(2020, 2, 1, 0, 0, 0, 0, time.UTC))
	assert.Nil(t, r2.Validate(ctx, "2010-01-02"))
	err := r2.Validate(ctx, "1999-01-02")
	if assert.NotNil(t, err) {
		assert.Equal(t, "the date is out of range", err.Error())
	}
	err = r2.Validate(ctx, "2021-01-02")
	if assert.NotNil(t, err) {
		assert.Equal(t, "the date is out of range", err.Error())
	}
}
