...
// A boolean value is also returned to indicate if
...
type thisCtxType int

var thisCtxKey thisCtxType = 1

func GetThis(ctx context.Context) any {
	if ctx == nil {
		return nil
	}
	if this, ok := ctx.Value(thisCtxKey).(any); ok {
		return this
	}
	return nil
}
...
func WithThis(ctx context.Context, this any) context.Context {
	...
	return context.WithValue(ctx, thisCtxKey, this)
}

// UpdateLevelChildrenNamesByModelTags recurrence updates the names of the children in the error level by using
// the provided model's tags.
func UpdateLevelChildrenNamesByModelTags(err error, model interface{}) error {
	var e *ErrorLevel
	if errors.As(err, &e) {
		e.UpdateChildrenNamesByModelTags(model)
		return e
	}
	return err
}
